...
class QuantumSearch:
    ...
    def _run_grovers_algorithm(
        ...
        try:
            ...
            # Filter marked items to only include valid indices within the
            # qubit address space.  Indices >= num_items would require more
            # qubits than allocated and produce incorrect oracle circuits.
            original_count = len(marked_items)
            marked_items = [i for i in marked_items if i < num_items]
            if len(marked_items) < original_count:
                logger.warning(
                    "Truncated %d marked items that exceeded qubit capacity (%d items, %d qubits)",
                    original_count - len(marked_items),
                    num_items,
                    num_qubits,
                )
