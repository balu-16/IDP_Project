...
class VectorStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        if self._initialized:
            # Warn if a different path is being requested
            requested = self._resolve_db_path(db_path or settings.CHROMA_DB_PATH)
            if requested != self.db_path:
                logger.warning(
                    "VectorStore singleton already initialized with path '%s'; "
                    "ignoring new path '%s'. Call VectorStore.reset_instance() to reinitialize.",
                    self.db_path,
                    requested,
                )
            return
        ...
        logger.info(f"VectorStore configured with path: {self.db_path}")

    @classmethod
    def reset_instance(cls) -> None:
        """Reset the singleton so the next constructor call creates a fresh instance.

        Useful for utility scripts (e.g. clear_chroma_db.py) that need an
        independent VectorStore pointing at a specific path.
        """
        cls._instance = None
