...
async def chat_message(
    ...
    try:
        ...
        user = await database.get_user_by_id(request.user_id)
        if not user:
        ...
        chat_id = str(request.chat_id) if request.chat_id is not None else (session_id or f"session_{datetime.utcnow().timestamp()}")
