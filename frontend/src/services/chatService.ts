...
class ChatService {
  ...
  async createNewSession(): Promise<ChatSession> {
    ...
    try {
      // Save to Supabase first and use the database-generated ID.
      // This avoids collisions between Date.now() and Supabase integer IDs.
      const chatId = await supabaseChatService.createChatSession(this.currentUserId, {
        id: '',  // placeholder — will be overwritten below
        title: 'New Chat',
