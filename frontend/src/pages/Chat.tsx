...
const Chat: React.FC = () => {
  ...
  const clearDatabase = async () => {
    try {
      const response = await fetch(apiUrl('/api/v1/clear_pdfs'), {
        method: 'DELETE',
        headers: {
          'Authorization': token ? `Bearer ${token}` : '',
        },
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        const uploadPromises = files.map(async (fileItem) => {
          ...
          try {
            const response = await fetch(apiUrl('/api/v1/upload_pdf'), {
              method: 'POST',
              headers: {
                'Authorization': token ? `Bearer ${token}` : '',
              },
    ...
    <div className="relative flex h-screen overflow-hidden">
      ...
      <div className={cn(
        ...
        <button
          ...
          className="p-2 rounded-lg bg-surface/80 border border-border backdrop-blur-sm text-text-secondary hover:text-text-primary transition-colors"
      ...
      {/* Main Chat Area */}
          ...
          <div className="w-full space-y-6">
            {messages.map((message) => (
              <ChatBubble
                ...
                retrievalTimeMs={message.retrievalTimeMs}
              />
            ))}
              ...
              >
                ...
                <div className="text-center">
                  <Button
                    onClick={() => setShowFileDropzone(false)}
                    variant="outline"
                    className="glass-panel border-border text-text-secondary"
                  >
                    Close
                  </Button>
                </div>
        ...
        <div className="shrink-0 border-t border-border bg-surface/90 dark:bg-glass-bg backdrop-blur-md shadow-soft px-6 pt-4 pb-6">
          <div className="w-full max-w-full">
            ...
            {isUploadingFiles && (
              <div className="mb-3">
                <div className="glass-panel px-4 py-3 border border-primary/30 rounded-lg bg-primary/10">
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <Input
                    ...
                    className="py-6 bg-transparent border-border focus:border-primary/50 focus:ring-primary/20 text-text-primary placeholder:text-text-secondary glass-panel shadow-none"
