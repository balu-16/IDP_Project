...
const DropdownMenu: React.FC<DropdownMenuProps> = ({ chatId, onRename, onDelete, onClose }) => {
  return (
    <motion.div
      ...
      className="absolute right-0 top-8 z-50 glass-panel border border-border rounded-xl p-1 min-w-[140px]"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        onClick={() => { onRename(chatId); onClose(); }}
        className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-surface/80 rounded-lg transition-colors"
      >
        <Edit size={14} />
        Rename
      </button>
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={cn(
          'h-screen backdrop-blur-md bg-surface/50 dark:bg-glass-bg border-r border-border z-50',
      ...
      >
        ...
        <div className="p-4 border-b border-border">
          ...
          <motion.button
            ...
            onClick={onNewChat}
            className={cn(
              "w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-medium transition-all shadow-sm glow-primary hover:glow-accent",
              ...
              <div key={chat.id} className="relative group">
                <motion.button
                  ...
                  className={cn(
                    ...
                    chat.isActive && 'bg-primary/10 border border-primary/30 glow-primary',
        ...
        <div className="p-3 border-t border-border space-y-1">
          <button
            ...
            className={cn(
              "w-full rounded-xl hover:bg-surface/80 text-text-secondary hover:text-text-primary transition-colors",
          ...
          <button
            ...
            className={cn(
              "w-full rounded-xl hover:bg-error-bg/20 text-text-secondary hover:text-error-text transition-colors",
              isCollapsed ? "p-3 flex items-center justify-center" : "flex items-center gap-3 p-3"
            )}
